...
def cut_and_save( R       : int
    ...
    if city == 'isaura':
        root_path_isaura = '/data/halmazan/NEXT/'
    else:
        root_path_isaura = root_path 
        # get a directory to save to
    folder_name = f'{root_path}PROCESSING/topology_cuts/data/{R}/{city}/{TS}/'
    folder_s = Path(f'{folder_name}')
    folder_s.mkdir(parents=True, exist_ok=True)

    # load in
    n100_dir = f'{root_path_isaura}N100_LPR/{R}/{city}/{TS}/'
    ...
    print('=' * 20)
    print(efficiencies)
    print(f'Events before: {hdst.event.nunique()}')
    print(f'Events after: {cut_hdst.event.nunique()}')
    # SAVE THE DATAFRAME
    cut_hdst.to_hdf(f'{folder_name}cut_hdst.h5', key = 'Tracking/Tracks')
    print(f'Data saved to {folder_name}cut_hdst.h5')
