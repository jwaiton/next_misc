...
def main(RUN_NUMBER, 
    ...
    # make directory
    pre_dir = '/scratch/halmazan/NEXT/N100_LPR/FOM/'
    folder_name = f'{pre_dir}'
    for rn, ts in zip(RUN_NUMBER, TIMESTAMP):
        print(f'Loading in R{rn}-{ts}')
        folder_name += f"{rn}-{ts}_"
    folder_name += f'{CITY}/'
    ...
    hdst_local = []
    for RN, TS in zip(RUN_NUMBER, TIMESTAMP):
        n100_dir = f'/scratch/halmazan/NEXT/N100_LPR/{RN}/{CITY}/{TS}/'
        hdst_local.append(load_data(n100_dir))
    hdst = pd.concat(hdst_local)
    # apply_cuts
    # removing low_E events (satellites)
    #elow_hdst = func.remove_low_E_events(hdst)
    elow_hdst = hdst
    cut_hdst, efficiency = func.apply_cuts(elow_hdst, lower_z = lower_z, upper_z = upper_z, r_lim = r_lim, lower_e = lower_e, upper_e = upper_e)
    print(efficiency)

    # save the efficiency table
    efficiency.to_csv(f'{folder_name}FOMefficiency.csv')

    vars = 'energy'
    labels = '(MeV)'
    log_flag = False
    print('=' * 20)
    print('data loaded, efficiency calculated')
    print(f'{efficiency}')
    ...
    def gaussian_fit(data, plot = False):
        ...
        print("Starting minimization with migrad...")
        # minimise it
        m_g.migrad()
    ...
    # [500, 1.65, 0.01]
    blob_data_fresh = cut_hdst.copy(deep = True)
    # scale accordingly
    blob_data_fresh.energy = blob_data_fresh.energy - 0.0608
    fit_range = [lower_e, upper_e]
    fit_range = [x - 0.0608 for x in fit_range]
    ...
    for i in range(len(cut_list)):
        ...
        try:
            ...
            try:
                ...
                print('Cant delete shit that doesnt exist i guess?')
    print(f'FOM:\n{fom}')
    print(f'Cut list:\n{cut_list}')
    try:
        plt.show()
    ...
    #plt.ylim([0,2])
    plt.ylabel("fom")
    plt.savefig(f'{folder_name}FOM/FOM_plot.pdf')
    plt.close()

    # save
    with open(f'{folder_name}FOM/FOM.csv', 'w') as f:
        writer = csv.writer(f)
...
if __name__ == '__main__':
    ...
    TIMESTAMP  = [str(x) for x in TIMESTAMP.split(',')]
    CITY       = sys.argv[3]
    lower_e    = float(sys.argv[4])
    upper_e    = float(sys.argv[5])
    lower_z    = float(sys.argv[6])
    upper_z    = float(sys.argv[7])
    r_lim      = float(sys.argv[8])

    main(RUN_NUMBER, TIMESTAMP, CITY, lower_e, upper_e, lower_z, upper_z, r_lim)
