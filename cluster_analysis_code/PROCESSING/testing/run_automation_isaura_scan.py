...
for file in "$INPUT_DIR"/*; do
	...
	echo "satellite_params = dict(
        satellite_start_iter = 5,
        satellite_max_size = 200,
        e_cut = {ecut},
        cut_type = CutType.rel)"                            >> ${{config_path}}
...
INPUT_DIR="/data/halmazan/NEXT/{s_path}/{RUN}/beersheba/{INPT_TIMESTAMP}/ldc{LDC}"
...
# parallel execution
cat "$CONFIG_LIST" | xargs -n 1 -P "$SLURM_CPUS_PER_TASK" -I{{}} city {CITY} {{}}
#seq 1000 | xargs -n 1 -P "$SLURM_CPUS_PER_TASK" -I{{}} z "${{OUTPUT_DIR}}/file_{{}}"
...
def run_city_isaura(TIMESTAMP, RUNS, CITY, JOB_TYPE, CORR_TS, MC, params):
    ...
    for i, RN in enumerate(RUNS):
        ...
        #print('SECOND BATCH')
        #with Pool() as pool:
        #    pool.map(process_ldc, range(5, 8))
...
def main():
    ...
    TSs             = [122035, 152035, 182035, 123045, 153045, 183045, 124055, 154055, 184055]
    #TSs             = [1005, 1015]
    #RUNS            = ['202510']
    JOB_TYPE        = 'beersheba'
    ...
    RUNS            = [15589, 15590]#[15589, 15590]#, 15591, 15592]
