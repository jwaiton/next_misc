...
def correct_and_cut(  R   : int
    ...
    file_out = f'/scratch/halmazan/NEXT/N100_LPR/{R}/sophronia/{TS}/'
    ...
    for i, f in enumerate(files):
        ...
        R, I = files[1].split('/')[-1].split('_')[1:3]
    ...
    with tb.open_file(f'{file_out}/run_{R}_ldc{LDC}_{TS}.h5', 'w', filters = tbl.filters(compression)) as h5out:
        df_writer(h5out,   passing_dsts, "DST", "Events" , compression="ZLIB4")
        df_writer(h5out,   passing_hits, "RECO", "Events" , compression="ZLIB4")
        df_writer(h5out,   passing_rInf, "Run" , "runInfo" , compression="ZLIB4")
        df_writer(h5out,   passing_evts, "Run" , "events", compression="ZLIB4")
